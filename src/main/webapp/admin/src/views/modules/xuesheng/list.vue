<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('xuesheng','成绩') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="kaoshichengjiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'成绩':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','成绩') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="kaoshichengjiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'成绩':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','成绩') && contents.tableBtnIcon == 0" type="success" size="mini" @click="kaoshichengjiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'成绩':'' }}</el-button>
    ...
    <kaoshichengji-cross-add-or-update v-if="kaoshichengjiCrossAddOrUpdateFlag" :parent="this" ref="kaoshichengjiCrossaddOrUpdate"></kaoshichengji-cross-add-or-update>
...
import kaoshichengjiCrossAddOrUpdate from "../kaoshichengji/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      kaoshichengjiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    kaoshichengjiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.kaoshichengjiCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.kaoshichengjiCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.kaoshichengjiCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
    },
